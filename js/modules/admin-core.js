...
const Pagination = {
  render(containerId, current, total, onPageChange) {
    ...
    if (start > 1) { html += `<button class="pagination-btn" onclick="${onPageChange}(1)">1</button>`; if (start > 2) html += '<span class="pagination-btn" style="cursor:default">...</span>'; }
    for (let i = start; i <= end; i++) html += `<button class="pagination-btn ${i === current ? 'active' : ''}" onclick="${onPageChange}(${i})">${i}</button>`;
    if (end < total) { if (end < total - 1) html += '<span class="pagination-btn" style="cursor:default">...</span>'; html += `<button class="pagination-btn" onclick="${onPageChange}(${total})">${total}</button>`; }
